...
class unit:
    def __init__(self,root):
        ...
        self.root.geometry('549x397+500+100')

        unitid=StringVar()
        unitname=StringVar()
        def ShowData():
            conn=mysql.connect(host='localhost',user='root',password='',database='db_bsp5a')
            cursor=conn.cursor()
            cursor.execute('select * from  tb_unit')
        ...
        def SaveData():
            conn = mysql.connect(host='localhost', user='root', password='', database='db_bsp5a')
            cursor = conn.cursor()
            cursor.execute('insert into tb_unit values(%s,%s)',(unitid.get(), unitname.get()))
            cursor.execute('commit')
            conn.colse()
            ShowData()
        ...
        txtUnitID=Entry(innerframe,font=fonts,textvariable=unitid,width=40)
        ...
        txtUnitName = Entry(innerframe, font=fonts, textvariable=unitname, width=40)
        ...
        self.member_record=ttk.Treeview(detailframe,height=7,columns=('UnitID','UnitName'),yscrollcommand=scroll_y.set)
